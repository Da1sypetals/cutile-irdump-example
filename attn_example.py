...
def compile_and_dump_ir():
    ...
    # 使用 mock tensor 进行编译
    q = MockTensor((seq_len, hidden_size), "float32")
    k = MockTensor((hidden_size, seq_len), "float32")
    v = MockTensor((seq_len, hidden_size), "float32")
    out = MockTensor((seq_len, hidden_size), "float32")

    pyfunc = flash_attention_forward_v2._pyfunc  # 获取原始 Python 函数
    ...
    try:
        ...
        try:
            ...
            mlir_path = "./ir_artifacts/flash_attention.cuda_tile.mlir"
